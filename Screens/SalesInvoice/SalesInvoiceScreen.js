...
const SalesInvoiceScreen = ({navigation}) => {
    ...
    const saveSalesInvoice = async () => {
        try{
            ...
            console.log("taxes",salesOrderTaxes);
            const invoiceTaxes = await db.getFirstAsync(`SELECT * FROM Sales_Taxes_and_Charges WHERE parent = ?`, [invoiceName]);
            console.log("Retrieved tax data after insert:", invoiceTaxes);
